...
void SimpleHeapMemory<T>::Allocate(const size_t& p_length) noexcept
{
  ...
  if (!m_memory)
  {
    NNE_ERORR( "new operation failed, this is a severe error!");
    return;
  }
...
void SimpleHeapMemory<T>::LoadFromFile(const char* p_file_path)
{
  ...
  if (input_file.fail())
  {
    NNE_ERORR("file path is not valid");
    return;
  }
  ...
  if (input_file.fail())
  {
    NNE_ERORR( "failed to get file size");
    return;
  }
  ...
  if (input_file.fail())
  {
    NNE_ERORR( "failed to read the file");
    return;
  }
